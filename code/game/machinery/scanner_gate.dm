		...
		if(SCANGATE_NUTRITION)
			if(ishuman(M))
				...
				if(H.nutrition <= detect_nutrition && detect_nutrition == NUTRITION_LEVEL_STARVING)
					beep = TRUE
				//if(H.nutrition >= detect_nutrition && detect_nutrition == NUTRITION_LEVEL_FAT)  ORBSTATION REMOVAL: no fat check
				//	beep = TRUE
