...
/datum/component/personal_crafting/proc/construct_item(atom/crafter, datum/crafting_recipe/recipe)
	...
	var/final_craft_time = HAS_TRAIT(crafter, TRAIT_FAST_CRAFTER) ? (recipe.time * FAST_CRAFTER_MOD) : recipe.time

	if(ismob(crafter) && !do_after(crafter, final_craft_time, target = crafter))
		return "."
